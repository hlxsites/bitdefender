...
function getPageName(pageIndex, path, part, current) {
...
export async function createBreadcrumbs() {
  ...
  const breadcrumbs = [
    ...
    ...pathSplit.slice(1, -1).map((part, index) => {
      ...
      return {
        name: getPageName(pageIndex, url, part, false),
        url_path: url,
      };
    }),
    {
      ...
      name: getPageName(pageIndex, pathname, pathSplit[pathSplit.length - 1], true),
    },
  ];
